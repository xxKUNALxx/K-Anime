...
const Watch = () => {
  ...
  const groupEpisodes = (episodes) => {
    ...
    for (let i = 0; i < episodes.length; i += 100) {
      grouped.push(episodes.slice(i, i + 100));
    }
    return grouped;
  };
  ...
  const groupedEpisodes = groupEpisodes(animeData.episodes);
        ...
        <div id="episodes">
          ...
          {groupedEpisodes.map((group, index) => (
            <div key={index} className="mb-4">
              <details className="bg-black bg-opacity-50 rounded-lg shadow-lg">
                <summary className="cursor-pointer py-2 px-4 text-lg font-medium bg-gray-800 rounded-t-lg">
                  Episodes {index * 100 + 1} - {index * 100 + group.length}
                </summary>
                <div className="episodelist-container flex overflow-x-scroll space-x-4 py-2 px-2">
                  {group.map((ep) => (
                    <div
                      className="episode-box bg-gray-700 rounded-lg p-4 hover:bg-indigo-600 transition-colors shadow-lg cursor-pointer transform hover:scale-105"
                      key={ep.id}
